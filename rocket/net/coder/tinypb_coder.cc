...
namespace rocket
{
    void TinyPBCoder::encode(std::vector<AbstractProtocal::s_ptr> &messages, TcpBuffer::s_ptr out_buffer)
    {
        for(auto &i:messages){
    ...
    void TinyPBCoder::decode(std::vector<AbstractProtocal::s_ptr> &out_message, TcpBuffer::s_ptr buffer)
    {
        // 便利buffer 找的pb__start 找到后解析出长度。然后得到结束符的位置判断是否为pb_end
        ...
        while (1)
        {
            ...
            if(i >= buffer->writeIndex()){
                DEBUGLOG("decode end, read all buf data");
                return;
            }
            ...
            if (parse_success)
            {
                ...
                DEBUGLOG("parse req_id_len_index=%d", message->m_req_id_len);

                int req_id_index = req_id_len_index + sizeof(message->m_req_id_len);

                // char req_id_index = req_id_len_index + sizeof(message->m_req_id_len);
